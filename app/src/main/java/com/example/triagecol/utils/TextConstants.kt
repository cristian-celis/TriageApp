package com.example.triagecol.utils

object TextConstants {
    const val USERNAME = "Usuario"
    const val PASSWORD = "Contraseña"
    const val WELCOME = "Bienvenido"
    const val VALIDATING = "Validando..."

    const val PHONE_NUMBER = "Numero de Telefono"

    const val LOGIN = "Iniciar Sesion"
    const val SIGN_OFF = "Cerrar Sesion"
    const val ADMIN_TITLE = "Administrador"
    const val NO_STAFF_REGISTERED = "No hay personal registrado."
    const val ADD_TEXT = "Agregar"

    const val EDIT_STAFF_TITLE = "Editar Personal"
    const val ADD_STAFF_TITLE = "Agregar Personal"
    const val DELETE_TEXT = "Eliminar"
    const val NOT_TEXT = "No"
    const val YES_TEXT = "Si"
    const val CONFIRM_DELETE_USER = "¿Esta seguro de eliminar al usuario?"

    const val CONFIRM_SIGN_OFF = "¿Esta seguro de cerrar sesion ahora?"

    const val CONFIRM_CHANGE_DOCTOR_STATUS = "¿Esta seguro que desea cambiar de estado?"
    const val CONFIRM_DELETE_ALL_PATIENTS = "¿Esta seguro que desea eliminar a TODOS los pacientes registrados?"
    const val SAVE_TEXT = "Guardar"
}